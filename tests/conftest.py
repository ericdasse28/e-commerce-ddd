...
def add_stock(postgres_session):
 ...
 eta)"
    ...
    for batch_id in batches_added:
        ...
        postgres_session.execute(
            text("DELETE FROM batches WHERE id=:sku"), {"batch_id": batch_id}
        )

    for sku in skus_added:
        postgres_session.execute(
            text("DELETE FROM order_lines WHERE sku=:sku"), {"sku": sku}
        )
        postgres_session.commit()
...
@pytest.fixture
def restart_api():
    (Path(__file__).parent.parent / "e_commerce_ddd/flask_app.py").touch()
    time.sleep(0.5)
